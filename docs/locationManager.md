...
```
Singleton class that wraps the native Mapbox location manager.Manages GPS location updates and distributes them to registered listeners.
...
Caches the last known location for immediate access.Use the default export locationManager rather than instantiating this class directly.
